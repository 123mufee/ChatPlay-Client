...
export default function Search() {
  ...
  const showProfile = async (id) => {
    await axios.get(`${postUrl}userProfile/${id}`).then((response) => {
  ...
  return (
    <>
      ...
      >
        ...
        <input
          type="text"
          className="md:mt-10 border-0 px-3 py-3 placeholder-blue-800 text-white bg-blue-500 rounded text-sm shadow focus:outline-none focus:ring w-full focus:shadow-xl hover:shadow-xl transition-shadow duration-300 ease-in-out"
        ...
        <ul>
          {results.length !== 0 && results.length !== 5 && (
            <>
              {results.map((result) => (
                <div key={result.id} className="bg-blue-600">
                  <h3
      ...
      {showUserModal2 ? <PeopleModal data={userData} /> : null}
    </>
  );
}
