...
export default function Search() {
  ...
  return (
    <>
      <div
        className={`top-0 right-0 w-full sm:w-[35vw] bg-purple-600  p-10 md:pl-20 sm:pl-0 text-white fixed h-full z-40  ease-in-out duration-300 fixed ${showSearch ? "translate-x-0 " : "translate-x-full"
          }`}
      >
        ...
        <input
          type="text"
          className="mt-10 border-0 px-3 py-3 placeholder-purple-800 text-white bg-purple-500 rounded text-sm shadow focus:outline-none focus:ring w-full focus:shadow-xl hover:shadow-xl transition-shadow duration-300 ease-in-out"
        ...
        <ul>
          {results.length !== 0 && results.length !== 5 && (
            <>
              {results.map((result) => (
                <div key={result._id} className="bg-purple-600">
                  <h3
                    className={`mt-10 text-3xl font-semibold text-white cursor-pointer inline-block duration-300 ease-in-out ${isHovered ? "hover:underline" : ""
                      }`}
                  ...
                  >
                    <div className="flex items-center">
                      <img src={result.image} alt={result.firstName} className="w-10 h-10 rounded-full mr-4" />
